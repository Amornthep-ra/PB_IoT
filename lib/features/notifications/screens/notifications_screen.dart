...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _loadData({bool showLoading = true}) async {
    ...
    final rules = List<AlertRuleModel>.from(
  ...
  Future<void> _toggleEventRead(AlertEventModel event) async {
    if (_isUpdatingEvents) {
      return;
    }

    final nextIsRead = !event.isRead;
    setState(() {
      _isUpdatingEvents = true;
      _events = _events
          .map(
            (entry) => entry.id == event.id
                ? entry.copyWith(isRead: nextIsRead)
                : entry,
          )
    ...
    try {
      await _notificationService.markEventRead(event.id, isRead: nextIsRead);
    } finally {
      if (mounted) {
        setState(() {
                  ...
                  children: [
                    ...
                    DecoratedBox(
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [
                          BoxShadow(
                            color: const Color(0xFFE28A3B).withValues(alpha: 0.24),
  ...
  Future<void> _openEventDetail(AlertEventModel event) async {
    await Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (_) => AlertEventDetailScreen(event: event),
      ),
    );
  }
  ...
  Future<void> _deleteRule(AlertRuleModel rule) async {
    ...
    final shouldDelete = await _confirmHistoryAction(
      ...
      message: 'ลบกฎแจ้งเตือน "${rule.title}" ใช่หรือไม่? หลังลบแล้วระบบจะไม่สร้างเหตุการณ์จากกฎนี้อีก',
      confirmLabel: 'Delete',
    );
    if (!shouldDelete || !mounted) {
      return;
          ...
          Column(
            children: [
              for (var index = 0; index < rules.length; index++) ...[
                _AlertRuleCard(
                  rule: rules[index],
                  sourceMissing: !availableWidgetIds.contains(rules[index].widgetId),
...
class _EventsSection extends StatelessWidget {
  ...
  final ValueChanged<AlertEventModel> onToggleRead;
      ...
      children: [
        Row(
          ...
          children: [
            ...
            const SizedBox(width: 12),
            if (allowClear && events.isNotEmpty)
              TextButton(
        ...
        if (events.isEmpty)
          _EmptyEventsCard(
            title: isHistorySection ? 'No saved history yet' : 'No current events',
            ...
            children: [
              for (var index = 0; index < events.length; index++) ...[
                _AlertEventCard(
                  ...
                  onToggleRead: () => onToggleRead(events[index]),
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      ...
                      _InfoChip(
                        label: _severityLabel(rule.severity),
                        color: palette.color,
                      ),
                      _InfoChip(
        ...
        child: Row(
          ...
          children: [
            Container(
              ...
              alignment: Alignment.center,
              child: Icon(palette.icon, color: palette.color, size: 24),
            ),
            const SizedBox(width: 14),
            Expanded(
              child: Column(
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      TextButton(
                        ...
                        child: Text(event.isRead ? 'Mark unread' : 'Mark read'),
                      ),
                    ],
                  ),
                ],
